...
from config import (
    ConfigManager, DEFAULT_HOTKEY_SCREENSHOT, DEFAULT_HOTKEY_LIVE_TRANSLATOR,
...
class FramioApp(QObject):
    def __init__(self):
        ...
        self.hotkey_mgr = GlobalHotkeyManager(
            ...
            hotkey_quick_drag_screenshot=getattr(self.cfg, "quick_drag_screenshot_hotkey", DEFAULT_HOTKEY_QUICK_DRAG_SCREENSHOT),
            hotkey_quick_drag_ocr=getattr(self.cfg, "quick_drag_ocr_hotkey", DEFAULT_HOTKEY_QUICK_DRAG_OCR)
        )
        ...
        self.hotkey_mgr.quick_drag_requested.connect(self.start_quick_drag)
        self.hotkey_mgr.quick_drag_mouse_up.connect(self.finish_quick_drag)
    ...
    def show_notification(
        ...
        if use_custom:
            ...
            QTimer.singleShot(0, lambda: ToastManager.instance().show_toast(
                title=title,
                message=message,
                icon_name=icon_name,
    ...
    def start_quick_drag(self, mode: str, x: int, y: int):
        self.overlay.start_quick_drag(mode, x, y)

    @pyqtSlot(int, int)
    def finish_quick_drag(self, x: int, y: int):
        self.overlay.finish_quick_drag(x, y)
    ...
    def _on_settings_applied(self):
        self.hotkey_mgr.update_hotkeys(
            ...
            live_translator=getattr(self.cfg, "hotkey_live_translator", ""),
            quick_drag_screenshot=getattr(self.cfg, "quick_drag_screenshot_hotkey", DEFAULT_HOTKEY_QUICK_DRAG_SCREENSHOT),
            quick_drag_ocr=getattr(self.cfg, "quick_drag_ocr_hotkey", DEFAULT_HOTKEY_QUICK_DRAG_OCR)
        )
        self._update_tray_state()
