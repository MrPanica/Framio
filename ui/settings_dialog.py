...
from config import (
    ...
    DEFAULT_HOTKEY_HIGHLIGHT_OBJECTS, DEFAULT_HOTKEY_LIVE_TRANSLATOR,
    DEFAULT_HOTKEY_QUICK_DRAG_SCREENSHOT, DEFAULT_HOTKEY_QUICK_DRAG_OCR,
...
def split_mouse_gesture(gesture_str: str) -> tuple[str, str]:
    """Разделяет строку комбинации клавиш и кнопки мыши на (keys, mouse_btn)."""
    if not gesture_str or not gesture_str.strip():
        return "", "LButton"
    parts = [p.strip() for p in gesture_str.split("+") if p.strip()]
    modifiers = []
    btn = "LButton"
    for p in parts:
        low = p.lower()
        if low in ("lbutton", "left", "лкм"):
            btn = "LButton"
        elif low in ("rbutton", "right", "пкм"):
            btn = "RButton"
        elif low in ("mbutton", "middle", "wheel", "скм"):
            btn = "MButton"
        elif low == "ctrl":
...
class SettingsDialog(QDialog):
    ...
    def _create_hotkeys_tab(self) -> QWidget:
        ...
        # 6. Плавающая рамка перевода субтитров
        w_trans = QWidget()
        l_trans = QHBoxLayout(w_trans)
        l_trans.setContentsMargins(0, 0, 0, 0)
        l_trans.setSpacing(6)
        self.edit_hotkey_translator = QLineEdit(getattr(self.cfg, "hotkey_live_translator", DEFAULT_HOTKEY_LIVE_TRANSLATOR))
        self.edit_hotkey_translator.setFixedWidth(170)
        self.edit_hotkey_translator.setPlaceholderText(tr("settings_hk_none", "Не назначено"))
        btn_rec_trans = HotkeyRecorderButton(self.edit_hotkey_translator)
        btn_rec_trans.setFixedWidth(82)
        btn_reset_trans = QPushButton(tr("settings_btn_clear", "Очистить"))
        btn_reset_trans.setFixedWidth(72)
        btn_reset_trans.clicked.connect(lambda: self.edit_hotkey_translator.setText(DEFAULT_HOTKEY_LIVE_TRANSLATOR))
        l_trans.addWidget(self.edit_hotkey_translator)
        l_trans.addWidget(btn_rec_trans)
        l_trans.addWidget(btn_reset_trans)
        card_hotkey.add_row(
            tr("settings_hk_translator", "Плавающая рамка перевода:"),
            tr("settings_hk_translator_desc", "Глобальная клавиша вызова экранного переводчика субтитров (по умолчанию не назначена)"),
            w_trans
        )

        layout.addWidget(card_hotkey)

        # Карточка: Быстрый захват мышью (Drag & Drop)
        card_mouse = SettingCard(tr("settings_mouse_gesture_group", "Быстрый захват мышью (Drag & Drop)"))

        # 1. Быстрый скриншот области мышью
        qd_screen_val = getattr(self.cfg, "quick_drag_screenshot_hotkey", DEFAULT_HOTKEY_QUICK_DRAG_SCREENSHOT)
        screen_keys, screen_btn = split_mouse_gesture(qd_screen_val)

        w_qd_screen = QWidget()
        l_qd_screen = QVBoxLayout(w_qd_screen)
        l_qd_screen.setContentsMargins(0, 0, 0, 0)
        l_qd_screen.setSpacing(6)

        row_qd_s1 = QHBoxLayout()
        row_qd_s1.setContentsMargins(0, 0, 0, 0)
        row_qd_s1.setSpacing(6)
        ...
        self.combo_qd_screen_btn = QComboBox()
        self.combo_qd_screen_btn.addItem(tr("settings_qd_btn_left", "Левая кнопка мыши (ЛКМ)"), "LButton")
        self.combo_qd_screen_btn.addItem(tr("settings_qd_btn_right", "Правая кнопка мыши (ПКМ)"), "RButton")
        self.combo_qd_screen_btn.addItem(tr("settings_qd_btn_middle", "Средняя кнопка мыши (СКМ / Колёсико)"), "MButton")
        idx_s_btn = self.combo_qd_screen_btn.findData(screen_btn)
        if idx_s_btn >= 0:
        ...
        row_qd_s1.addWidget(btn_rec_qd_screen)
        row_qd_s1.addWidget(QLabel("+"))
        row_qd_s1.addWidget(self.combo_qd_screen_btn)
        row_qd_s1.addWidget(btn_reset_qd_screen)
        row_qd_s1.addStretch()
        l_qd_screen.addLayout(row_qd_s1)

        row_qd_s2 = QHBoxLayout()
        row_qd_s2.setContentsMargins(0, 0, 0, 0)
        row_qd_s2.setSpacing(6)
        lbl_s_fmt = QLabel(tr("settings_qd_format", "Формат буфера:"))
        lbl_s_fmt.setStyleSheet("color: #94a3b8; font-size: 11px;")
        self.combo_qd_screen_fmt = QComboBox()
        self.combo_qd_screen_fmt.addItem("PNG", "png")
        self.combo_qd_screen_fmt.addItem("JPEG / JPG", "jpg")
        self.combo_qd_screen_fmt.addItem("WebP", "webp")
        cur_s_fmt = getattr(self.cfg, "quick_drag_screenshot_format", "png").lower()
        idx_sf = self.combo_qd_screen_fmt.findData(cur_s_fmt)
        if idx_sf >= 0:
            self.combo_qd_screen_fmt.setCurrentIndex(idx_sf)
        self.combo_qd_screen_fmt.setFixedWidth(130)
        row_qd_s2.addWidget(lbl_s_fmt)
        row_qd_s2.addWidget(self.combo_qd_screen_fmt)
        row_qd_s2.addStretch()
        l_qd_screen.addLayout(row_qd_s2)

        card_mouse.add_row(
            tr("settings_qd_screenshot", "Быстрый скриншот области:"),
            tr("settings_qd_screenshot_desc", "Зажмите комбинацию клавиш + кнопку мыши на экране, выделите область и отпустите для моментального снимка в буфер"),
            w_qd_screen
        )

        # 2. Быстрое распознавание текста (OCR) мышью
        qd_ocr_val = getattr(self.cfg, "quick_drag_ocr_hotkey", DEFAULT_HOTKEY_QUICK_DRAG_OCR)
        ocr_keys, ocr_btn = split_mouse_gesture(qd_ocr_val)

        w_qd_ocr = QWidget()
        l_qd_ocr = QVBoxLayout(w_qd_ocr)
        l_qd_ocr.setContentsMargins(0, 0, 0, 0)
        l_qd_ocr.setSpacing(6)

        row_qd_o1 = QHBoxLayout()
        row_qd_o1.setContentsMargins(0, 0, 0, 0)
        row_qd_o1.setSpacing(6)
        ...
        self.combo_qd_ocr_btn = QComboBox()
        self.combo_qd_ocr_btn.addItem(tr("settings_qd_btn_left", "Левая кнопка мыши (ЛКМ)"), "LButton")
        self.combo_qd_ocr_btn.addItem(tr("settings_qd_btn_right", "Правая кнопка мыши (ПКМ)"), "RButton")
        self.combo_qd_ocr_btn.addItem(tr("settings_qd_btn_middle", "Средняя кнопка мыши (СКМ / Колёсико)"), "MButton")
        idx_o_btn = self.combo_qd_ocr_btn.findData(ocr_btn)
        if idx_o_btn >= 0:
        ...
        row_qd_o1.addWidget(btn_rec_qd_ocr)
        row_qd_o1.addWidget(QLabel("+"))
        row_qd_o1.addWidget(self.combo_qd_ocr_btn)
        row_qd_o1.addWidget(btn_reset_qd_ocr)
        row_qd_o1.addStretch()
        l_qd_ocr.addLayout(row_qd_o1)

        row_qd_o2 = QHBoxLayout()
        row_qd_o2.setContentsMargins(0, 0, 0, 0)
        row_qd_o2.setSpacing(6)
        lbl_o_lang = QLabel(tr("settings_qd_lang", "Язык распознавания:"))
        lbl_o_lang.setStyleSheet("color: #94a3b8; font-size: 11px;")
        self.combo_qd_ocr_lang = QComboBox()
        self.combo_qd_ocr_lang.addItem(tr("settings_lang_auto", "Автоматически"), "auto")
        try:
            for l_info in get_available_ocr_languages():
                self.combo_qd_ocr_lang.addItem(f"{l_info['name']} ({l_info['tag']})", l_info["tag"])
        except Exception:
            pass
        cur_o_lang = getattr(self.cfg, "quick_drag_ocr_lang", "auto")
        idx_ol = self.combo_qd_ocr_lang.findData(cur_o_lang)
        if idx_ol >= 0:
            self.combo_qd_ocr_lang.setCurrentIndex(idx_ol)
        self.combo_qd_ocr_lang.setFixedWidth(200)
        row_qd_o2.addWidget(lbl_o_lang)
        row_qd_o2.addWidget(self.combo_qd_ocr_lang)
        row_qd_o2.addStretch()
        l_qd_ocr.addLayout(row_qd_o2)

        card_mouse.add_row(
            tr("settings_qd_ocr", "Быстрое распознавание текста (OCR):"),
            tr("settings_qd_ocr_desc", "Зажмите комбинацию клавиш + кнопку мыши, выделите текст и отпустите — распознанный текст сразу скопируется в буфер"),
            w_qd_ocr
        )

        layout.addWidget(card_mouse)
    ...
    def _create_general_tab(self) -> QWidget:
        ...
        self.chk_custom_notif = QCheckBox(tr("settings_custom_notif", "Компактные всплывающие уведомления Framio над панелью задач"))
    ...
    def _apply_settings(self):
        ...
        if hasattr(self, "edit_qd_screen_keys") and hasattr(self, "combo_qd_screen_btn"):
            k_screen = self.edit_qd_screen_keys.text().strip()
            btn_screen = self.combo_qd_screen_btn.currentData() or "LButton"
            self.cfg.quick_drag_screenshot_hotkey = join_mouse_gesture(k_screen, btn_screen)
        if hasattr(self, "combo_qd_screen_fmt"):
            self.cfg.quick_drag_screenshot_format = self.combo_qd_screen_fmt.currentData() or "png"

        if hasattr(self, "edit_qd_ocr_keys") and hasattr(self, "combo_qd_ocr_btn"):
            k_ocr = self.edit_qd_ocr_keys.text().strip()
            btn_ocr = self.combo_qd_ocr_btn.currentData() or "LButton"
            self.cfg.quick_drag_ocr_hotkey = join_mouse_gesture(k_ocr, btn_ocr)
        if hasattr(self, "combo_qd_ocr_lang"):
            self.cfg.quick_drag_ocr_lang = self.combo_qd_ocr_lang.currentData() or "auto"
