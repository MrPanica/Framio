...
class OverlayWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.quick_drag_mode = None
        self.quick_drag_format = "png"
        self.quick_drag_ocr_lang = "auto"
    ...
    def start_capture(self, preselected_recording_mode: str = None):
        ...
        # 2. Отрисовываем новый снимок экрана и обрабатываем события отрисовки ДО включения видимости,
        # чтобы QBackingStore гарантированно отправил свежий кадр в DWM / UpdateLayeredWindow
        self.update()
        for _ in range(2):
            QApplication.processEvents()
    ...
    def start_quick_drag(self, mode: str, x: int, y: int):
        """Запускает режим быстрого выделения области мышью (скриншот или OCR)."""
        self.start_capture()
        self.quick_drag_mode = mode
        self.quick_drag_format = getattr(self.cfg, "quick_drag_screenshot_format", "png")
        self.quick_drag_ocr_lang = getattr(self.cfg, "quick_drag_ocr_lang", "auto")
        self.is_selecting = True
        self.drag_start_pos = QPointF(float(x), float(y))
        self.selection_rect = QRectF(self.drag_start_pos, self.drag_start_pos)
        self._hide_toolbars()
        self.setCursor(Qt.CursorShape.CrossCursor)
        try:
            self.grabMouse()
    ...
    def finish_quick_drag(self, x: int, y: int):
        """Завершает быстрый захват по сигналу отпускания мыши от глобального хука."""
        self._finish_quick_drag(QPointF(float(x), float(y)))

    def _finish_quick_drag(self, pos: QPointF = None):
        """Обрабатывает завершение быстрого захвата области мышью."""
        try:
            self.releaseMouse()
        except Exception:
            pass

        if not getattr(self, "quick_drag_mode", None):
            return

        mode = self.quick_drag_mode
        fmt = getattr(self, "quick_drag_format", "png")
        ocr_lang = getattr(self, "quick_drag_ocr_lang", "auto")
        self.quick_drag_mode = None
        self.is_selecting = False

        try:
            if pos is not None and getattr(self, "drag_start_pos", None) is not None:
            ...
            if rect.width() >= 10 and rect.height() >= 10 and self.background_pixmap is not None:
                crop = self.background_pixmap.copy(rect)
                if mode == "screenshot":
                    clipboard = QApplication.clipboard()
                    if fmt == "jpg":
                        self._set_images_on_clipboard([crop.toImage()], "jpg")
                    elif fmt == "webp":
                    ...
                    if getattr(self.cfg, "play_sound", True):
                        play_capture_sound()
                    self._notify(
                        title=tr("notif_quick_screen_copied", "Скриншот скопирован в буфер"),
                        message=f"{fmt.upper()} · {rect.width()}×{rect.height()} px",
                        thumbnail_pixmap=crop,
                        copy_data=crop,
                        icon_name="camera"
                    )
                elif mode == "ocr":
                    ...
                    if text and isinstance(text, str) and text.strip():
                        ...
                        self._notify(
                            title=tr("notif_ocr_copied_title", "Текст скопирован в буфер обмена"),
                            message=clean_text[:140],
                            thumbnail_pixmap=crop,
                            copy_data=clean_text,
                            icon_name="scan_text"
                        )
                    else:
                        self._notify(
                            title=tr("notif_ocr_no_text_title", "Текст не найден"),
                            message=tr("notif_ocr_no_text_body", "На выбранной области изображения текст не обнаружен."),
                            thumbnail_pixmap=crop,
                            icon_name="scan_text"
                        )
        except Exception as e:
            print(f"[QuickDrag] Error in finish_quick_drag: {e}")
        finally:
            self.close_overlay()
    ...
    def mouseReleaseEvent(self, event):
        ...
        if getattr(self, "quick_drag_mode", None):
            self._finish_quick_drag(pos)
            return
    ...
    def save_screenshot(self, fmt="png", all_regions: bool = False):
        ...
        if path:
            ...
            try:
                ...
                print(f"[Overlay] Save notify error: {e}")
            if preserve_selection:
                self._restore_single_recording_overlay_after_action()
            else:
                self.close_overlay()
    ...
    def copy_screenshot(self, fmt="standard", all_regions: bool = False):
        ...
        if not preserve_selection:
            # Мгновенно скрываем оверлей: нулевая задержка закрытия зоны на экране (0 мс)
            self.setWindowOpacity(0.0)
            self._hide_toolbars()
            self.badge.hide()
            self.hide()
        ...
        try:
            ...
            print(f"[Overlay] Copy notify error: {e}")
        if preserve_selection:
            self._restore_single_recording_overlay_after_action()
        else:
            self.close_overlay()
    ...
    def copy_ocr_text(self, lang="auto", all_regions: bool = False):
        ...
        if full_text:
            ...
            try:
                ...
                print(f"[Overlay] OCR notify error: {e}")
        else:
            try:
                self._notify(
