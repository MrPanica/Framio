    ...
    def _finish_quick_drag(self, pos: QPointF = None):
        ...
        if rect.width() >= 10 and rect.height() >= 10 and self.background_pixmap is not None:
            ...
            if mode == "screenshot":
                ...
                text = extract_text_from_image(pil_img, lang=ocr_lang)
                ...
                if text and text.strip():
                    ...
                    self._notify(
                        ...
                        message=text.strip()[:140],
                        thumbnail_pixmap=crop,
                        copy_data=text.strip(),
                        icon_name="scan_text"
                    )
                else:
                    self._notify(
    ...
    def save_screenshot(self, fmt="png", all_regions: bool = False):
        ...
        if path:
            ...
            self._notify(
                ...
                thumbnail_pixmap=images[0] if images else None,
                copy_data=images[0] if images else None,
                icon_name="save"
            )
            if preserve_selection:
                self._restore_single_recording_overlay_after_action()
